  ...
  ReentrancyGuardUpgradeable
{
  ...
  modifier onlyDao() {
    require(msg.sender == owner() || msg.sender == address(_dao), "Dao: not allowed");
    _;
  }
  ...
  function provide() external payable override nonReentrant returns (uint256 value) {
  ...
  function provideInABNBc(uint256 amount) external override nonReentrant returns (uint256 value) {
    ...
    returns (uint256 realAmount)
  {
    ...
    require(amount >= minumunUnstake, "value must be greater than min unstake amount");
  ...
  function liquidation(address recipient, uint256 amount) external override onlyDao nonReentrant {
    _ceRouter.withdrawABNBc(recipient, amount);
  }

  function daoBurn(address account, uint256 value) external override onlyDao nonReentrant {
    _collateralToken.burn(account, value);
  }

  function daoMint(address account, uint256 value) external override onlyDao nonReentrant {
