...
xdescribe("===MVP1===", () => {
  ...
  let jug: Jug;
  let dog: Dog;
  let clipABNBC: Clipper;
  let abaci: LinearDecrease;
  let vow: Vow;
  let interaction: DAOInteraction;
  let rewards: HelioRewards;
  let helio: HelioToken;
  let oracle: Oracle;

  const wad = "000000000000000000"; // 18 Decimals
  const ray = "000000000000000000000000000"; // 27 Decimals
  const rad = "000000000000000000000000000000000000000000000"; // 45 Decimals
  const ONE = 10 ** 27;

  const collateral = ethers.utils.formatBytes32String("aBNBc");
  const collateral2 = ethers.utils.formatBytes32String("aBNBc2");
  ...
  before(async () => {
    ...
    const ABNBC = await ether.getContractFactory("ABNBc");
    ...
    const ClipABNBC = await ethers.getContractFactory("Clipper");
    ...
    const Interaction = await ethers.getContractFactory("DAOInteraction"); // Mock Oracle
    const Rewards = await ethers.getContractFactory("HelioRewards");
    const Helio = await ethers.getContractFactory("HelioToken");
    ...
    gemJoin = await GemJoin.connect(deployer).deploy(vat.address, collateral, abnbc.address);
    ...
    await dog.deployed();
    clipABNBC = await ClipABNBC.connect(deployer).deploy(
      vat.address,
      spot.address,
      dog.address,
      collateral
    ...
    abaci = await Abaci.connect(deployer).deploy();
    await abaci.deployed();

    // System Stabilizer module (balance sheet)
    vow = await Vow.connect(deployer).deploy(vat.address, NULL_ADDRESS, NULL_ADDRESS, NULL_ADDRESS);
    await vow.deployed();
    ...
    rewards = await Rewards.connect(deployer).deploy(vat.address);
    await rewards.deployed();
    helio = await Helio.connect(deployer).deploy();
    await helio.deployed();

    interaction = await Interaction.connect(deployer).deploy();
    await interaction
      .connect(deployer)
      .initialize(
        vat.address,
    ...
    await rewards
      .connect(deployer)
      .initPool(helio.address, collateral, "1000000001847694957439350500"); //6%
    //////////////////////////////
    /** Initial Setup -------- **/
    //////////////////////////////
    ...
    await vat
      .connect(deployer)
      ["file(bytes32,uint256)"](ethers.utils.formatBytes32String("Line"), "20000" + rad); // Normalized USB
    await vat
      .connect(deployer)
    ...
    await spot
      ...
      ["file(bytes32,bytes32,uint256)"](
        collateral,
        ethers.utils.formatBytes32String("mat"),
        "1250000000000000000000000000"
      ); // Liquidation Ratio
    await spot
      .connect(deployer)
      ["file(bytes32,uint256)"](ethers.utils.formatBytes32String("par"), "1" + ray); // It means pegged to 1$
    ...
    await proxyLike
      ...
      .jugInitFile(
        collateral,
        ethers.utils.formatBytes32String("duty"),
        "0000000000312410000000000000"
      ); // 1% Yearly Factored

    expect(await (await jug.base()).toString()).to.be.equal("1000000000315529215730000000");
    expect(await (await (await jug.ilks(collateral)).duty).toString()).to.be.equal(
      "312410000000000000"
    );

    await jug
      .connect(deployer)
      ["file(bytes32,address)"](ethers.utils.formatBytes32String("vow"), vow.address);
    ...
    await dog
      .connect(deployer)
      ["file(bytes32,uint256)"](ethers.utils.formatBytes32String("Hole"), "500" + rad);
    await dog
      .connect(deployer)
      ["file(bytes32,bytes32,uint256)"](
        collateral,
    ...
    await clipABNBC
      .connect(deployer)
      ["file(bytes32,uint256)"](ethers.utils.formatBytes32String("tail"), "1800"); // 30mins reset time
    await clipABNBC
      .connect(deployer)
      ["file(bytes32,uint256)"](
    ...
    await clipABNBC
      .connect(deployer)
      ["file(bytes32,uint256)"](ethers.utils.formatBytes32String("chip"), "10000000000000000"); // 1% from vow incentive
    await clipABNBC
      .connect(deployer)
      ["file(bytes32,uint256)"](ethers.utils.formatBytes32String("tip"), "10" + rad); // 10$ flat fee incentive
    await clipABNBC
      .connect(deployer)
      ["file(bytes32,uint256)"](ethers.utils.formatBytes32String("stopped"), "0");
    await clipABNBC
      .connect(deployer)
      ["file(bytes32,address)"](ethers.utils.formatBytes32String("spotter"), spot.address);
    await clipABNBC
      .connect(deployer)
      ["file(bytes32,address)"](ethers.utils.formatBytes32String("dog"), dog.address);
    await clipABNBC
      .connect(deployer)
      ["file(bytes32,address)"](ethers.utils.formatBytes32String("vow"), vow.address);
    await clipABNBC
      .connect(deployer)
      ["file(bytes32,address)"](ethers.utils.formatBytes32String("calc"), abaci.address);

    await (<Contract>abaci.connect(deployer))["file(bytes32,uint256)"](
      ethers.utils.formatBytes32String("tau"),
      "3600"
    ); // Price will reach 0 after this time
  ...
  it("should check collateralization and borrowing Usb", async function () {
    ...
    expect((await vat.connect(deployer).gem(collateral, signer1.address)).toString()).to.be.equal(
      await ethers.utils.parseEther("400").toString()
    );
    expect((await vat.connect(deployer).gem(collateral, signer2.address)).toString()).to.be.equal(
      await ethers.utils.parseEther("900").toString()
    );
    ...
    await vat
      ...
      .frob(
        ...
        0
      ); // 400 * 1.6$ = 640$ worth locked
    await network.provider.send("evm_mine");
    expect((await vat.connect(deployer).gem(collateral, signer1.address)).toString()).to.be.equal(
      await ethers.utils.parseEther("0").toString()
    );
    expect(
      (await (await vat.connect(deployer).urns(collateral, signer1.address)).ink).toString()
    ).to.be.equal(await ethers.utils.parseEther("400").toString());
    ...
    await vat
      ...
      .frob(
        ...
        0
      ); // 900 * 1.6$ = 1440$ worth locked
    await network.provider.send("evm_mine");
    expect((await vat.connect(deployer).gem(collateral, signer2.address)).toString()).to.be.equal(
      await ethers.utils.parseEther("0").toString()
    );
    expect(
      (await (await vat.connect(deployer).urns(collateral, signer2.address)).ink).toString()
    ).to.be.equal(await ethers.utils.parseEther("900").toString());
    ...
    expect(
      (await (await vat.connect(signer1).urns(collateral, signer1.address)).art).toString()
    ).to.be.equal("0");
    ...
    let debt_rate = (await vat.ilks(collateral)).rate;
    let usb_amount1 = BigNumber.from("550000000000000000000").div(debt_rate).mul(ONE);
    console.log("HERE");
    console.log(usb_amount1);
    let usb_amount2 = BigNumber.from("600000000000000000000").div(debt_rate).mul(ONE);
    ...
    console.log(
      "art(signer1)  : " +
        (await (
          await (
            await vat.connect(signer1).urns(collateral, signer1.address)
          ).art
        ).toString())
    );
    console.log(
      "Art           : " +
        (await (await (await vat.connect(signer1).ilks(collateral)).Art).toString())
    );
    console.log(
      "Usb(signer1)  : " + (await (await vat.connect(signer1).usb(signer1.address)).toString())
    );
    console.log("Debt          : " + (await (await vat.connect(signer1).debt()).toString()));

    await interaction
      .connect(deployer)
      .enableCollateralType(abnbc.address, gemJoin.address, collateral, clipABNBC.address);
    ...
    console.log("Debt          : " + (await (await vat.connect(signer1).debt()).toString()));
    let usbWithStabilityFee = debt_rate
      .mul((await vat.connect(signer1).urns(collateral, signer1.address)).art)
      .div(ONE); // rate * art = usb
    let stabilityFee = usbWithStabilityFee
      .sub(await vat.connect(signer1).usb(signer1.address))
      .div(ONE); // S.fee = usbWithStabilityFee - usb
    ...
    expect((await usb.balanceOf(signer1.address)).toString()).to.equal(
    ...
    usb_amount1 = BigNumber.from("-550000000000000070000");
    await vat
      .connect(signer1)
      .frob(
        collateral,
    ...
    await expect(
      dog.connect(deployer).bark(collateral, signer2.address, signer3.address)
    ...
    expect(
      await (await (await vat.connect(signer2).urns(collateral, signer2.address)).art).toString()
    ).to.equal("0");
    ...
    await clipABNBC
      .connect(signer1)
      .take("1", "3" + wad, "2200000000000000000000000000", signer1.address, "0x");
  ...
  it("Interaction test", async function () {
    ...
    await interaction
      .connect(signer1)
      .deposit(signer1.address, abnbc.address, ether("1200").toString());
    ...
    await interaction
      .connect(signer1)
      .borrow(signer1.address, abnbc.address, ether("600").toString());
    let borrowed = await interaction.connect(signer1).borrowed(abnbc.address, signer1.address);
    ...
    await interaction
      .connect(signer1)
      .deposit(signer1.address, abnbc.address, ether("10").toString());
    await interaction
      .connect(signer1)
      .borrow(signer1.address, abnbc.address, ether("600").toString());
    borrowed = await interaction.connect(signer1).borrowed(abnbc.address, signer1.address);
